...
public class Templates {
    ...
    private static String replacedVariablesTemplate(String template, Map<String, String> variables) {
        ...
        var matcher = VARIABLES_REGEX.matcher(template);

        var replaced = template;

        while (matcher.find()) {
            var varGroup = matcher.group(0);
            var varName = matcher.group(1);
            var varValue = variables.get(varName);
            if (varValue == null) {
                System.out.printf("Variable %s not supplied, leaving it as it is\n", varName);
            } else {
                replaced = replaced.replace(varGroup, varValue);
            }
        }
